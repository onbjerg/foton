...
fn color_for_ray(ray: &Ray, scene: &Scene, depth: u8) -> Vec3 {
    ...
    match scene.hit(&ray, 0.001, std::f32::MAX) {
        Some(ref hit) if depth < 50 => hit
            .material
            .scatter(&ray, hit)
            .map_or(Vec3::new(0.0, 0.0, 0.0), |scatter| {
                scatter.attenuation * color_for_ray(&scatter.ray, &scene, depth + 1)
            }),
